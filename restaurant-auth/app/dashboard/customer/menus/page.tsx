...
export default function ManageMenusPage() {
  ...
  const [imagePreview, setImagePreview] = useState<string>(''); // สำหรับโชว์ให้ดูเฉยๆ
  const [imageFile, setImageFile] = useState<File | null>(null); // สำหรับไฟล์ตัวจริงที่จะส่งไป Backend
  ...
  const handleOpenEdit = (menu: Menu) => {
    ...
    setImagePreview(menu.image || ''); // โชว์รูปเดิมจาก URL
    setImageFile(null); // เคลียร์ไฟล์ เผื่อไม่ได้อัปโหลดใหม่
    setIsEditing(true);
    setIsModalOpen(true);
  };
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setImageFile(file); // เก็บไฟล์จริงเตรียมส่งให้ API
      setImagePreview(URL.createObjectURL(file)); // สร้าง URL จำลองให้โชว์บนหน้าเว็บทันที
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // 📦 ใช้ FormData แทน JSON เพื่อให้แนบ "ไฟล์รูป" ได้
      const formData = new FormData();
      if (formId) formData.append('id', formId.toString());
      formData.append('name', formName);
      formData.append('price', formPrice.toString());
      if (imageFile) formData.append('image', imageFile); // แนบไฟล์รูปภาพตัวจริงไป

      const method = isEditing ? 'PUT' : 'POST';
      
      const res = await fetch('/api/shop/menus', {
        method,
        // 🚨 ห้ามใส่ headers: { 'Content-Type': 'application/json' } เด็ดขาด!
        // เพราะเบราว์เซอร์จะจัดการ Header สำหรับ FormData ให้เองโดยอัตโนมัติ
        body: formData, 
      });
  ...
  const toggleRecommended = async (id: number, currentStatus: boolean) => {
    try {
      await fetch('/api/shop/menus', {
        method: 'PATCH', // แนะนำให้ใช้ PATCH สำหรับแก้ข้อมูลบางส่วน
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, is_recommended: !currentStatus }),
      });
      fetchMenus();
    } catch (error) {
      console.error("Error toggling recommend status");
    }
  };
